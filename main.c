...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define TRIGGER_DISTANCE 100 //Jarak menembak dalam cm
#define PUMP_PIN GPIO_BSRR_BS0 //PIN untuk mengontrol pompa
...
#define LIDAR_SENSOR_PIN    GPIO_PIN_4  // Pin untuk sensor LIDAR
#define MOTOR_PIN           GPIO_PIN_5  // Pin untuk motor
#define LIDAR_THRESHOLD 100  // Jarak ambang batas untuk berhenti (misalnya, dalam milimeter)
#define LIDAR_SENSOR_PIN GPIO_PIN_6  // Pin untuk sensor LIDAR
#define MOTOR_PIN        GPIO_PIN_7  // Pin untuk motor
/* USER CODE END PD */
...
uint16_t GetLidarDistance(void) {
    ...
    return 120; // Ini adalah nilai jarak dalam milimeter
}
...
int main(void)
{
  ...
  while (1)
  {
	  	  	  int targetDetected = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
	 		  if (targetDetected){
	 			  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1); //mulai tembak
	 			  HAL_Delay(1000); // delay 1 detik
	 			  HAL_TIM_PWM_Stop(&htim1, TIM_CHANNEL_1); //berhenti tembak
	 		  }
...
void SystemClock_Config(void)
{
  ...
  /** Configure the mai n internal regulator output voltage
  */
